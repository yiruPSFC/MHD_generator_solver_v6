...
_DEFAULT_JX_IN_GUESS = 32000.0


def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Run a progressively relaxed continuation schedule for the v2 NLP workflow, "
            "with inlet variables optimized inside the CasADi problem"
        )
    )
    p.add_argument("--np-in-guess", type=float, default=3.05e25)
    p.add_argument("--np-in-min", type=float, default=1.5e25)
    p.add_argument("--np-in-max", type=float, default=6.0e25)
    p.add_argument("--te-in-guess", type=float, default=4420.0)
    p.add_argument("--te-in-min", type=float, default=2200.0)
    p.add_argument("--te-in-max", type=float, default=6500.0)
    p.add_argument("--z-in-guess", type=float, default=75.954994)
    p.add_argument("--z-in-min", type=float, default=20.0)
    p.add_argument("--z-in-max", type=float, default=120.0)
    p.add_argument("--jx-in-guess", type=float, default=_DEFAULT_JX_IN_GUESS)
    p.add_argument("--jx-in-min", type=float, default=1000.0)
    p.add_argument("--jx-in-max", type=float, default=100000.0)
    ...
    p.add_argument(
        "--out-dir",
        type=str,
        default=str(_CASADI_V2_DIR / "outputs" / "continuation" / "relaxed_reference_case_v2"),
...
def main() -> int:
    ...
    payload["inlet_margin_mode"] = str(args.inlet_margin_mode)
    payload["warm_profile_npz"] = str(args.warm_profile_npz)
    if args.out_json:
