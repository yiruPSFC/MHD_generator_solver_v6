...
Jeffrey Freidberg ${ }^{\text {® }}$<br>Plasma Science and Fusion Center, MIT, Cambridge, MA, 02139, USA<br>Corresponding author: Jeffrey Freidberg, jpfreid@mit.edu

(Received 12 November 2024; revision received 7 May 2025; accepted 8 May 2025)
...
With this as background, one can now ask why it makes sense to revisit the Velikhov-ionisation instability. There are three reasons, one societal, the other two, technical. The societal reason is as follows. At present, as compared with the 1990s, there is a much greater concern about climate change, particularly the production of $\mathrm{CO}_{2}$. Combined cycle gas, which produces approximately one half the $\mathrm{CO}_{2} /$ Watt as a coal plant, still generates enormous quantities of $\mathrm{CO}_{2}$. If the world is going to phase out natural gas in favour of power sources that produce no $\mathrm{CO}_{2}$, then nuclear fission, while still not highly popular but becoming more acceptable, and nuclear fusion, highly popular but not here yet, represent sources of baseload electricity satisfying this goal. Because of the high capital cost of either fission or fusion power plants, the efficiency gains resulting from an effective MHD topping cycle would be a big win.
...
the stability boundary for a Hall MHD convertor, which is the primary interest as discussed later, recasts the temperature difference into to a maximum limit on the Hall parameter, defined as $\beta=\Omega_{e} / \nu_{M}=\left(e B_{0} / m_{e}\right) / \nu_{M} \propto B_{0}$. Here, $\Omega_{e}$ is the electron cyclotron frequency and $\nu_{M}$ is the electron-primary gas momentum exchange collision frequency. The stability boundary sets a limit on the maximum allowable magnetic field: $\beta \leq \beta_{\text {crit }}$. Consequently, having access to higher fields above this limit would not appear to be of much use. It is of basic interest to learn which effect dominates - improved performance due to higher fields versus the high-field limit due to the instability?
...
(iv) The slice location with the most severe constraint depends, in general, on the shape of the channel. One strategy, discussed later in the analysis, is to design
the channel shape so that the marginal stability criterion is satisfied locally along the entire length of the generator. Under this strategy, the ionisation instability actually determines the input boundary conditions on the generator.
...
If the seed plasma is weakly ionised, then by definition, the electron density $n_{e}$ is small compared with the original seed density $n_{s}: n_{e} \ll n_{s}$. Thus, in accordance with Saha's ionisation equation, which determines $n_{e}$, the positive electron temperature perturbation will cause a corresponding increase in electron density. This density increase leads to a decrease in the electrical resistivity. Specifically, if electron-primary gas momentum collisions $v_{e p}$ are dominant, then the total momentum exchange collision frequency $\nu_{M} \approx \nu_{e p}$ and the electrical resistivity scales as

$$
\begin{equation*}
\eta=\frac{m_{e} \nu_{M}}{e^{2} n_{e}} \approx \frac{m_{e} \nu_{e p}}{e^{2} n_{e}} \approx \frac{m_{e} n_{p} \bar{\sigma}_{e p} v_{T e}}{e^{2} n_{e}} \propto \frac{T_{e}^{1 / 2}}{n_{e}} . \tag{2.1}
\end{equation*}
$$

Here, $n_{p}$ is the primary gas number density, $\bar{\sigma}_{e p}$ is the electron-primary gas collision cross-section and $v_{T e}=\left(2 k T_{e} / m_{e}\right)^{1 / 2}$ is the electron thermal velocity. Because of the strong exponential $T_{e}$ behaviour in the Saha equation, the resulting $n_{e}\left(T_{e}\right)$ density increase dominates and the resistivity decreases.
...
$$
\begin{array}{ll}
\boldsymbol{\nabla} \cdot\left(n_{p} \boldsymbol{v}_{p}\right)=0 & \text { Mass, } \\
m_{p} n_{p} \boldsymbol{v}_{p} \cdot \boldsymbol{\nabla} \boldsymbol{v}_{p}=\boldsymbol{J} \times \boldsymbol{B}_{0}-\boldsymbol{\nabla} p_{p} & \text { Momentum, } \\
\frac{3}{2} \boldsymbol{\nabla} \cdot\left(p_{p} \boldsymbol{v}_{p}\right)+p_{p} \boldsymbol{\nabla} \cdot \boldsymbol{v}_{p}=\frac{3}{2} v_{E} n_{e}\left(k T_{e}-k T_{p}\right) & \text { Energy. } \tag{3.1}
\end{array}
$$

### 3.2. Electron fluid

$\frac{n_{e}^{2}}{n_{s}-n_{e}}=\left(\frac{2 \pi m_{e} k T_{e}}{h^{2}}\right)^{3 / 2} \exp \left(-E_{I} / k T_{e}\right) \quad$ Mass - Saha equation,
$\boldsymbol{E}+\boldsymbol{v}_{p} \times \boldsymbol{B}_{0}=\eta \boldsymbol{J}+\frac{\boldsymbol{J} \times \boldsymbol{B}_{0}}{e n_{e}} \quad$ Momentum $-\mathrm{Ohm}^{\prime}$ s law,
$\frac{3}{2} v_{E} n_{e}\left(k T_{e}-k T_{p}\right)=\eta J^{2} \quad$ Energy - Electron energy balance.

### 3.3. Maxwell

$$
\begin{array}{ll}
\boldsymbol{\nabla} \cdot \boldsymbol{J}=0 & \text { Ampere's law } \\
\boldsymbol{\nabla} \times \boldsymbol{E}=0 & \text { Faraday's law } \\
n_{i}=n_{e} & \text { Poisson's equation - charge neutrality. } \tag{3.3}
\end{array}
$$

Note that the primary and electron fluid variables are denoted by the subscript ' $p$ ' and ' $e$ ', respectively. The quantity $E_{I}$ is the ionisation potential of the seed gas, $n_{s}$ is the density of the initial unionised seed gas and $\boldsymbol{B}_{0}$ is the applied vacuum magnetic field. The resistivity $\eta$ and temperature equilibration frequency $\nu_{E}$ for a monatomic gas are related to the electron momentum exchange frequency $\nu_{M}$ in the usual way,

$$
\begin{align*}
v_{M} & =v_{e p}+v_{e n}+v_{e i} \approx v_{e p}, \\
\eta & =\frac{m_{e} v_{M}}{n_{e} e^{2}}, \\
v_{E} & =2 \frac{m_{e}}{m_{p}} v_{M} . \tag{3.4}
\end{align*}
$$

Here, the separate contributions to $v_{M}$ represent electron collisions with (a) the primary gas ( $v_{e p}$ ), (b) the actual unionised seed particles ( $v_{e n}$ ) and (c) the seed ions $\left(v_{e i}\right)$. Some discussion of the physics is warranted.
(i) The use of fluid equations assumes that all collision frequencies are large compared with the characteristic MHD time scale: $v_{E} \gg v_{p} / L$ with $L$ the macroscopic generator length scale. This condition is well satisfied in all regimes of interest.
(ii) With respect to momentum collisions, the electron-primary gas interactions dominate in the operational regime of interest: $\nu_{e p} \gg \nu_{e n}, \nu_{e i}$. For mathematical simplicity, one thus assumes $v_{M} \approx v_{e p}$ in the following analysis. Even so, note that the analysis is generalised to include the full $\nu_{M}$, and some results are included for comparison. Also, an expression for $\nu_{e p}$ is given shortly.
...
$$
\begin{equation*}
v_{i} \approx v_{n} \approx v_{p}, \quad T_{i} \approx T_{n} \approx T_{p} . \tag{3.5}
\end{equation*}
$$

The neutral and ionised seed particles are dragged along and thermally equilibrated with the much denser primary gas. After a short calculation using the mass conservation equations (including ionisation and recombination) for the heave species, it can be easily shown that the quantity $n_{s}$ satisfies $\boldsymbol{\nabla} \cdot\left(n_{s} \boldsymbol{v}_{p}\right)=0$. Therefore, assuming that all species have the same uniform cross-sectional profile at the generator inlet, it follows that

$$
\begin{equation*}
\frac{n_{s}}{n_{s 0}}=\frac{n_{p}}{n_{p 0}}, \tag{3.6}
\end{equation*}
$$

with a ' 0 ' subscript denoting inlet value. This is the simple, required information relating $n_{s}$ to the basic unknown $n_{p}$.
(i) The next point of interest is the assumption that only the vacuum magnetic field enters the analysis. This too is a very good approximation since simple scaling relations show that the induced magnetic field $B_{\text {ind }}$ due to the generator currents is much smaller than the vacuum field: $B_{\text {ind }} \ll B_{0}$. One consequence of this approximation is that the only non-trivial information contained in Ampere's law is the relation $\boldsymbol{\nabla} \cdot \boldsymbol{J}=0$.
...
The first step in the analysis is to examine the equilibrium properties of the plasma. In addition to expressing all quantities in terms of the input velocity and magnetic
field, it will also be of use in the stability analysis to derive a relationship between the temperature difference, Hall parameter and ionisation fraction. As stated, the analysis focuses primarily on the behaviour of the electrons because of the $k_{0} L \gg 1$ assumption.
...
$$
\begin{align*}
E_{x} & =\eta J_{x}+\frac{J_{y} B_{0}}{e n_{e}}, \\
-v_{p} B & =\eta J_{y}-\frac{J_{x} B_{0}}{e n_{e}}, \\
E_{z} & =0 . \tag{3.7}
\end{align*}
$$

As assumed, the fluid velocity, $\boldsymbol{v}_{p} \approx v_{p} \boldsymbol{e}_{x}$, of the primary gas will remain unchanged during the development of the instability.

Next, one introduces definitions of several basic parameters entering the analysis,

$$
\begin{array}{rlr}
v_{M}=v_{e p}=n_{p} \bar{\sigma}_{e p} v_{T e}=n_{p} \bar{\sigma}_{e p}\left(\frac{2 k T_{e}}{m_{e}}\right)^{1 / 2} & \text { Collision frequency, } \\
\eta=\frac{m_{e} v_{M}}{e^{2} n_{e}} & \text { Resistivity, } \\
\beta=\frac{B_{0}}{e n_{e} \eta}=\frac{\Omega_{e}}{v_{M}} & \text { Hall parameter, } \\
M=\left(\frac{3}{5} \frac{m_{p} v_{p}^{2}}{k T_{p}}\right)^{1 / 2} & \text { Mach number. } \tag{3.8}
\end{array}
$$

Here, $\bar{\sigma}_{e p}$ is the known cross-section for momentum exchange collisions between the electrons and the primary gas. Also, the Mach number is important since the flow must be supersonic, $M>1$, for good energy conversion in a Hall MHD generator.

In addition, define an equilibrium load resistivity $\eta_{L}$ as

$$
\begin{equation*}
\eta_{L}(x)=-\frac{E_{x}}{J_{x}} \tag{3.9}
\end{equation*}
$$
...
$$
\begin{align*}
J_{x} & =\left[\frac{\beta^{2}}{\beta^{2}+1+Z}\right] e n_{e} v_{p}, \\
J_{y} & =-\frac{1+Z}{\beta} J_{x}=-\left[\frac{\beta(1+Z)}{\beta^{2}+1+Z}\right] e n_{e} v_{p}, \\
\frac{E_{x}}{\eta} & =-Z J_{x}=-\left[\frac{\beta^{2} Z}{\beta^{2}+1+Z}\right] e n_{e} v_{p}, \\
\frac{E_{y}}{\eta} & =0 . \tag{3.11}
\end{align*}
$$
...
$$
\begin{align*}
& S_{L}=-\boldsymbol{E} \cdot \boldsymbol{J}=-E_{x} J_{x}=m_{e} n_{e} v_{M} v_{p}^{2} \frac{\beta^{4} Z}{\left(\beta^{2}+1+Z\right)^{2}} \\
& S_{\Omega}=\eta J^{2}=\eta\left(J_{x}^{2}+J_{y}^{2}\right)=m_{e} n_{e} v_{M} v_{p}^{2} \frac{\beta^{2}\left[\beta^{2}+(1+Z)^{2}\right]}{\left(\beta^{2}+1+Z\right)^{2}} \\
& S_{C}=S_{L}+S_{\Omega}=m_{e} n_{e} v_{M} v_{p}^{2} \frac{\beta^{2}(1+Z)}{\beta^{2}+1+Z} \tag{3.12}
\end{align*}
$$
...
$$
\begin{equation*}
\frac{3}{2} v_{E} n_{e}\left(k T_{e}-k T_{p}\right)=\eta J^{2} \tag{3.13}
\end{equation*}
$$
...
$$
\begin{equation*}
\frac{T_{e}}{T_{p}}-1=\frac{2 \eta J^{2}}{3 v_{E} n_{e} k T_{p}}=\left(\frac{5 M^{2}}{9}\right) \frac{\beta^{2}\left[\beta^{2}+(1+Z)^{2}\right]}{\left(\beta^{2}+1+Z\right)^{2}} \tag{3.14}
\end{equation*}
$$
...
The stability analysis is carried out by linearising all quantities about their equilibrium values using a standard multiple length scale expansion. Assume a slow length scale $\boldsymbol{r}_{s}$ to describe variations along the long equilibrium length scale. Similarly,
introduce a fast length scale $\boldsymbol{r}_{f}$ to describe the short wavelength variations associated with the instability. The implication is that any quantity $Q_{\text {Tot }}(\boldsymbol{r}, t)$ can be linearised as follows:

$$
\begin{align*}
Q_{\text {Tot }}(\boldsymbol{r}, t) & =Q(\boldsymbol{r})+\tilde{Q}(\boldsymbol{r}, t) \\
& =Q\left(\boldsymbol{r}_{s}\right)+\tilde{Q}\left(\boldsymbol{r}_{f}, \boldsymbol{r}_{s}, t\right) \\
& =Q\left(\boldsymbol{r}_{s}\right)+\tilde{Q}\left(\boldsymbol{r}_{s}\right) \exp \left(\gamma t+i k_{x} x_{f}+i k_{y} y_{f}+i k_{z} z_{f}\right) . \tag{4.1}
\end{align*}
$$

Here, quantities with a tilde are the linearised perturbations and $\gamma \rightarrow 0$ corresponds to marginal stability.

A key mathematical point is to distinguish the different length scales between equilibrium and stability. This corresponds to the following ordering requirement: $\nabla Q \sim \partial Q / \partial \boldsymbol{r}_{s} \sim Q / L$ and $\nabla \tilde{Q} \sim \partial \tilde{Q} / \partial \boldsymbol{r}_{s}+\partial \tilde{Q} / \partial \boldsymbol{r}_{f} \sim \tilde{Q} / L+i k_{0} \tilde{Q} \approx i k_{0} \tilde{Q}$ with $k_{0}^{2}=k_{x}^{2}+k_{y}^{2}+k_{z}^{2}$. Because of the short wavelength assumption, each equilibrium quantity $Q\left(\boldsymbol{r}_{s}\right)$ can be viewed as a 'constant' on the short $\boldsymbol{r}_{f}$ stability length scale. The derivation of the stability boundary thus becomes purely algebraic (i.e. no differential equations to solve). It has the form of a local criterion that must be satisfied separately at each point $\boldsymbol{r}_{s}$ along the MHD channel. Violation at any location will lead to the development of a short wavelength ionisation instability.
...
$$
\begin{equation*}
\frac{\tilde{T}_{e}}{T_{e}}=2 \alpha \frac{\tilde{n}_{e}}{n_{e}}, \tag{4.2}
\end{equation*}
$$
...
$$
\begin{align*}
\alpha & =\frac{1}{2} \frac{N}{T_{e}\left(\mathrm{~d} N / \mathrm{d} T_{e}\right)}=\frac{1}{2}\left(\frac{2 k T_{e}}{3 k T_{e}+2 E_{I}}\right)\left(\frac{2-f_{I}}{1-f_{I}}\right) \\
& \approx\left(\frac{k T_{e}}{2 E_{I}}\right)\left(\frac{2-f_{I}}{1-f_{I}}\right) \propto \frac{1}{1-f_{I}} \tag{4.3}
\end{align*}
$$

and $f_{I}=n_{e} / n_{s}$ is the equilibrium fraction of seed ionisation. The approximate expression makes use of the fact that $k T_{e} / E_{I} \ll 1$. The value of $\alpha$ is critical in that it is a direct measure of the degree of ionisation of the seed gas. Small $\alpha \ll 1$ implies small to modest ionisation fraction: $f_{I}<1$. Moderate to large $\alpha>1$ implies nearly full ionisation: $f_{I} \rightarrow 1$. Also, as stated, the perturbed $\tilde{n}_{s}$ should be, and is, neglected.

### 4.2. The perturbed collision frequency

In the analysis, it is necessary to evaluate the perturbed collision frequency plus several related quantities. Consider first the momentum exchange collision frequency defined by $\nu_{M}=\nu_{e p}$. A straightforward calculation leads to

$$
\begin{equation*}
\tilde{v}_{M}=\tilde{v}_{e p}=n_{p} \bar{\sigma}_{p} v_{T e}\left(\frac{1}{2} \frac{\tilde{T}_{e}}{T_{e}}\right)=v_{e p}\left(\frac{1}{2} \frac{\tilde{T}_{e}}{T_{e}}\right) . \tag{4.4}
\end{equation*}
$$

Next, $\tilde{T}_{e} / T_{e}$ from (4.2) is substituted, yielding the required relation between $\tilde{v}_{M}$ and $\tilde{n}_{e}$,

$$
\begin{equation*}
\frac{\tilde{v}_{M}}{v_{M}}=\alpha \frac{\tilde{n}_{e}}{n_{e}} . \tag{4.5}
\end{equation*}
$$
...
$$
\begin{equation*}
\frac{\tilde{\beta}}{\beta}=\frac{\left(\Omega_{e} / \nu_{M}\right)\left(-\tilde{v}_{M} / \nu_{M}\right)}{\left(\Omega_{e} / \nu_{M}\right)}=-\frac{\tilde{v}_{M}}{\nu_{M}}=-\alpha \frac{\tilde{n}_{e}}{n_{e}} . \tag{4.6}
\end{equation*}
$$

Similarly, one can easily derive the perturbed resistivity. Since the resistivity is defined as $\eta=m_{e} \nu_{M} / n_{e} e^{2}$, this leads to

$$
\begin{equation*}
\frac{\tilde{\eta}}{\eta}=\frac{\tilde{\nu}_{M}}{\nu_{M}}-\frac{\tilde{n}_{e}}{n_{e}}=(\alpha-1) \frac{\tilde{n}_{e}}{n_{e}} . \tag{4.7}
\end{equation*}
$$
...
$$
\begin{equation*}
\frac{\tilde{v}_{E}}{v_{E}}=\frac{\tilde{v}_{M}}{v_{M}}=\alpha \frac{\tilde{n}_{e}}{n_{e}} . \tag{4.8}
\end{equation*}
$$
...
$$
\begin{align*}
\nabla \cdot \boldsymbol{J} & =0 \rightarrow k_{x} \tilde{J}_{x}+k_{y} \tilde{J}_{y}+k_{z} \tilde{J}_{z}=0 \\
\nabla \times \boldsymbol{E} & =0 \rightarrow k_{x} \tilde{E}_{y}-k_{y} \tilde{E}_{x}=0 \quad \text { and } \quad k_{x} \tilde{E}_{z}-k_{z} \tilde{E}_{x}=0 \tag{4.9}
\end{align*}
$$

Note that the term $\partial \boldsymbol{B} / \partial t$ has been neglected in Faraday's law based on the wellsatisfied assumption that the induced currents in both equilibrium and stability are small, plus the focus on marginal stability. The magnetic field is a pure static vacuum field.
...
$$
\begin{align*}
\tilde{E}_{y} & =\frac{k_{y}}{k_{x}} \tilde{E}_{x}, \\
\tilde{E}_{z} & =\frac{k_{z}}{k_{x}} \tilde{E}_{x}, \\
\tilde{J}_{z} & =\frac{k_{z}}{k_{x}} \frac{\tilde{E}_{x}}{\eta}, \\
\tilde{J}_{y} & =-\frac{k_{x}}{k_{y}} \tilde{J}_{x}-\frac{k_{z}^{2}}{k_{x} k_{y}} \frac{\tilde{E}_{x}}{\eta} . \tag{4.10}
\end{align*}
$$
...
$$
\begin{align*}
& \left(1-\beta \frac{k_{x}}{k_{y}}\right) \tilde{J}_{x}-\left(1+\frac{\beta k_{z}^{2}}{k_{x} k_{y}}\right) \frac{\tilde{E}_{x}}{\eta}=\left[-(\alpha-1) J_{x}+\beta J_{y}\right] \frac{\tilde{n}_{e}}{n_{e}} \\
& \left(\frac{k_{x}}{k_{y}}+\beta\right) \tilde{J}_{x}+\left(\frac{k_{y}}{k_{x}}+\frac{k_{z}^{2}}{k_{x} k_{y}}\right) \frac{\tilde{E}_{x}}{\eta}=\left[(\alpha-1) J_{y}+\beta J_{x}\right] \frac{\tilde{n}_{e}}{n_{e}} \tag{4.11}
\end{align*}
$$
...
$$
\begin{gather*}
\tilde{J}_{x}=\frac{1}{1+\beta^{2}\left(k_{z}^{2} / k_{0}^{2}\right)}\left\{\begin{array}{l}
(\alpha-1) \frac{k_{y}}{k_{0}} J_{\perp}+\beta \frac{k_{y}}{k_{0}} J_{\|} \\
\left.+\frac{k_{z}^{2}}{k_{0}^{2}}\left[\left(\beta^{2}-\alpha+1\right) J_{x}+\alpha \beta J_{y}\right]\right\} \frac{\tilde{n}_{e}}{n_{e}}
\end{array}\right. \\
\frac{\tilde{E}_{x}}{\eta}=-\frac{1}{1+\beta^{2}\left(k_{z}^{2} / k_{0}^{2}\right)} \frac{k_{x}}{k_{0}}\left[\left(\beta^{2}-\alpha+1\right) J_{\|}+\alpha \beta J_{\perp}\right] \frac{\tilde{n}_{e}}{n_{e}} \\
\tilde{J}_{y}=\frac{1}{1+\beta^{2}\left(k_{z}^{2} / k_{0}^{2}\right)}\left\{\begin{array}{l}
-(\alpha-1) \frac{k_{x}}{k_{0}} J_{\perp}-\beta \frac{k_{x}}{k_{0}} J_{\|} \\
\left.+\frac{k_{z}^{2}}{k_{0}^{2}}\left[\left(\beta^{2}-\alpha+1\right) J_{y}-\alpha \beta J_{x}\right]\right\} \frac{\tilde{n}_{e}}{n_{e}}
\end{array}\right. \\
J_{x} \tilde{J}_{x}+J_{y} \tilde{J}_{y}=\frac{1}{1+\beta^{2}\left(k_{z}^{2} / k_{0}^{2}\right)}\left[-(\alpha-1) J_{\perp}^{2}-\beta J_{\perp} J_{\|}+\frac{k_{z}^{2}}{k_{0}^{2}}\left(\beta^{2}-\alpha+1\right) J^{2}\right] \frac{\tilde{n}_{e}}{n_{e}} .
\end{gather*}
$$
...
& J^{2}=J_{x}^{2}+J_{y}^{2}=\left(J_{\perp}^{2}+J_{\|}^{2}\right) /\left(1-k_{z}^{2} / k_{0}^{2}\right) \\
& J_{z}=0 \\
& J_{\perp}=\frac{1}{k_{0}}\left(\boldsymbol{e}_{z} \cdot \boldsymbol{k} \times \boldsymbol{J}\right)=\frac{1}{k_{0}}\left(k_{x} J_{y}-k_{y} J_{x}\right) \\
& J_{\|}=\frac{1}{k_{0}}(\boldsymbol{k} \cdot \boldsymbol{J})=\frac{1}{k_{0}}\left(k_{x} J_{x}+k_{y} J_{y}\right) \tag{4.13}
\end{align*}
$$
...
$$
\begin{equation*}
E_{I} \frac{\partial n_{e}}{\partial t}=\eta J^{2}-\frac{3}{2} v_{E} n_{e}\left(k T_{e}-k T_{p}\right) \tag{4.14}
\end{equation*}
$$
...
$$
\begin{equation*}
\gamma E_{I} \tilde{n}_{e}=J^{2} \tilde{\eta}+2 \eta\left(J_{x} \tilde{J}_{x}+J_{y} \tilde{J}_{y}\right)-\frac{3}{2}\left(k T_{e}-k T_{p}\right)\left(\tilde{v}_{E} n_{e}+v_{E} \tilde{n}_{e}\right)-\frac{3}{2} v_{E} n_{e} k \tilde{T}_{e} . \tag{4.15}
\end{equation*}
$$
...
$$
\begin{equation*}
\left(\gamma n_{e} E_{I}\right) \frac{\tilde{n}_{e}}{n_{e}}=\left\{\eta J^{2}(\alpha-1+2 K)-\frac{3}{2} n_{e} \nu_{E}\left[(3 \alpha+1)\left(k T_{e}-k T_{p}\right)+2 \alpha k T_{P}\right]\right\} \frac{\tilde{n}_{e}}{n_{e}}, \tag{4.16}
\end{equation*}
$$
...
$$
\begin{equation*}
K\left(k_{x}, k_{y}, k_{z}\right)=\frac{1}{1+\beta^{2}\left(k_{z}^{2} / k_{0}^{2}\right)}\left[-(\alpha-1) \frac{J_{\perp}^{2}}{J^{2}}-\beta \frac{J_{\perp} J_{\|}}{J^{2}}+\frac{k_{z}^{2}}{k_{0}^{2}}\left(\beta^{2}-\alpha+1\right)\right] . \tag{4.17}
\end{equation*}
$$
...
$$
\begin{equation*}
\eta J^{2}(\alpha-1+2 K)-\frac{3}{2} n_{e} \nu_{E}\left[(3 \alpha+1)\left(k T_{e}-k T_{p}\right)+2 \alpha k T_{P}\right] \leq 0 \quad \text { for stability. } \tag{4.18}
\end{equation*}
$$
...
$$
\begin{equation*}
\frac{\tilde{n}_{e}}{n_{e}} \sim \frac{\tilde{T}_{e}}{T_{e}} \sim \frac{\tilde{J}_{x}}{J_{x}} \sim \frac{\tilde{J}_{y}}{J_{y}} . \tag{4.19}
\end{equation*}
$$
...
$$
\begin{equation*}
\frac{\tilde{n}_{p}}{n_{p}} \sim \frac{\tilde{v}_{p}}{v_{p}} \sim \frac{\tilde{T}_{p}}{T_{p}} \sim \frac{1}{k_{0} L} \frac{\tilde{n}_{e}}{n_{e}} . \tag{4.20}
\end{equation*}
$$
...
We begin with determining the most unstable wavenumber. Observe that the individual wavenumber components only appear in the coefficient $K=K\left(k_{x}, k_{y}, k_{z}\right)$. The sign of the inequality in (4.18) is such that the most unstable mode corresponds
to choosing the components of the wavenumber to make $K=K_{\max }$. This requires some tedious algebra and the following procedure is a useful strategy.
...
\begin{align*}
& k_{z}=k_{0} \sin \phi \\
& k_{x}=k_{0} \sin \zeta \cos \phi \\
& k_{y}=k_{0} \cos \zeta \cos \phi \\
& k_{x}^{2}+k_{y}^{2}+k_{z}^{2}=k_{0}^{2} \tag{5.1}
\end{align*}
$$
...
$$
\begin{equation*}
K=\frac{1}{1+\beta^{2} \sin ^{2} \phi}\left\{\left[-(\alpha-1) \frac{J_{\perp}^{2}}{J^{2}}-\beta \frac{J_{\perp} J_{\|}}{J^{2}}\right]+\left(\beta^{2}-\alpha+1\right) \sin ^{2} \phi\right\}, \tag{5.2}
\end{equation*}
$$
...
$$
\begin{align*}
& J_{\|}=\frac{\boldsymbol{k} \cdot \boldsymbol{J}}{k_{0}}=\frac{k_{x} J_{x}+k_{y} J_{y}}{k_{0}}=\left(J_{x} \sin \zeta+J_{y} \cos \zeta\right) \cos \phi, \\
& J_{\perp}=\frac{\boldsymbol{e}_{z} \cdot \boldsymbol{k} \times \boldsymbol{J}}{k_{0}}=\frac{k_{x} J_{y}-k_{y} J_{x}}{k_{0}}=\left(-J_{x} \cos \zeta+J_{y} \sin \zeta\right) \cos \phi, \\
& J_{\|}^{2}+J_{\perp}^{2}=\left(J_{x}^{2}+J_{y}^{2}\right) \cos ^{2} \phi=J^{2} \cos ^{2} \phi . \tag{5.3}
\end{align*}
$$
...
$$
\begin{align*}
J_{\|} & =-J \cos \chi \cos \phi, \\
J_{\perp} & =J \sin \chi \cos \phi . \tag{5.4}
\end{align*}
$$

The negative sign in $J_{\|}$implies that a positive $\chi$ maximises rather than minimises $K$. The relation between $\chi$ and $\zeta$ is given by

$$
\begin{equation*}
\tan \chi=\frac{J_{x}-J_{y} \tan \zeta}{J_{x} \tan \zeta+J_{y}} . \tag{5.5}
\end{equation*}
$$
...
$$
\begin{equation*}
K=\frac{1}{1+\beta^{2} \sin ^{2} \phi}\left\{\left[-(\alpha-1) \sin ^{2} \chi+\beta \sin \chi \cos \chi\right] \cos ^{2} \phi+\left(\beta^{2}-\alpha+1\right) \sin ^{2} \phi\right\} . \tag{5.6}
\end{equation*}
$$
...
$$
\begin{equation*}
\tan 2 \chi=\frac{\beta}{\alpha-1} \tag{5.7}
\end{equation*}
$$
...
$$
\begin{equation*}
K=\frac{\left\{\left[(\alpha-1)^{2}+\beta^{2}\right]^{1 / 2}-(\alpha-1)\right\} \frac{1-\sin ^{2} \phi}{2}+\left(\beta^{2}-\alpha+1\right) \sin ^{2} \phi}{1+\beta^{2} \sin ^{2} \phi} . \tag{5.8}
\end{equation*}
$$
...
$$
\begin{align*}
K & =K_{0} \frac{1+\hat{\beta}^{2} \sin ^{2} \phi}{1+\beta^{2} \sin ^{2} \phi} \\
\hat{\beta}^{2} & =\frac{\beta^{2}-\alpha+1}{K_{0}}-1 \\
K_{0} & =\frac{1}{2}\left\{\left[(\alpha-1)^{2}+\beta^{2}\right]^{1 / 2}-(\alpha-1)\right\} \geq 0 \tag{5.9}
\end{align*}
$$
...
$$
\begin{equation*}
\hat{\beta}^{2} \leq \beta^{2} \rightarrow-\left[2(\alpha-1)-\left(\beta^{2}-1\right)\right]^{2} \leq 0 . \tag{5.10}
\end{equation*}
$$

Clearly, this condition is always satisfied, showing that $\phi=0$ is the most unstable value. This corresponds to $k_{z}=0$, which agrees with intuition. Thus, the most unstable wavenumber leads to a value for $K=K_{\text {max }}$ given by

$$
\begin{equation*}
K_{\max }=K_{0}=\frac{1}{2}\left\{\left[(\alpha-1)^{2}+\beta^{2}\right]^{1 / 2}-(\alpha-1)\right\} \tag{5.11}
\end{equation*}
$$

The stability condition is now obtained by substituting $K_{\text {max }}$ from (5.11) into the general stability relation given by (4.18). This leads to

$$
\begin{equation*}
\eta J^{2}\left[(\alpha-1)^{2}+\beta^{2}\right]^{1 / 2}-\frac{3}{2} n_{e} \nu_{E}\left[(3 \alpha+1)\left(k T_{e}-k T_{p}\right)+2 \alpha k T_{P}\right] \leq 0 . \tag{5.12}
\end{equation*}
$$
...
$$
\begin{align*}
\beta^{2} & \leq 4 \alpha\left(2+\frac{1}{\Delta T}\right)\left[1+\alpha\left(1+\frac{1}{\Delta T}\right)\right], \\
\Delta T & =\frac{T_{e}}{T_{p}}-1, \\
\alpha & =\frac{k T_{e}}{2 E_{I}} \frac{2-f_{I}}{1-f_{I}} . \tag{5.13}
\end{align*}
$$
...
$$
\begin{equation*}
\frac{S_{\Omega}}{S_{L}}=\frac{\text { Ohmic power density }}{\text { Load power density }} \tag{6.1}
\end{equation*}
$$
...
$$
\begin{equation*}
\sigma=1 / \eta=\frac{n_{e} e^{2}}{m_{e} v_{M}}, \tag{6.2}
\end{equation*}
$$
...
An open cycle fossil fuel generator is characterised by a weakly ionised seed gas that is at nearly the same temperature as the primary gas. To obtain a high
electrical conductivity, the temperature must be relatively high ( $T_{e} \approx T_{p} \sim 2500 \mathrm{~K}$ ). The need for a high temperature can lead to material problems, which are one of the drawbacks of an open cycle fossil fuel system.

Keep in mind that the analysis so far has focused on closed cycle systems, which assume a monatomic gas. For a fossil fuel system, essentially all of the analysis still applies, with one exception. The relation between $v_{E}$ and $v_{M}$ is modified as follows:

$$
\begin{equation*}
v_{E}=2 \frac{m_{e}}{m_{p}} v_{M} \rightarrow v_{E}=2 \delta \frac{m_{e}}{m_{p}} v_{M} . \tag{6.3}
\end{equation*}
$$

Here, $\delta$ is an energy equilibration enhancement factor whose value depends on the primary gas used in the generator (see for instance Rosa (1987)). For a closed cycle system using a monatomic gas (e.g. Ar), then $\delta=1$. For an open cycle system using a mixture of complex molecular gases (e.g. $\mathrm{CO}, \mathrm{CO} 2, \mathrm{NO}$ ), then $\delta>200$. When this effect is included in the analysis, the important place in which it appears is in the relationship between $\Delta T$ and $Z$. Specifically, this relationship, given by (3.14), is now replaced by

$$
\begin{equation*}
\Delta T=\left(\frac{5 M^{2}}{9}\right) \frac{\beta^{2}\left[\beta^{2}+(1+Z)^{2}\right]}{\left(\beta^{2}+1+Z\right)^{2}} \rightarrow \Delta T=\left(\frac{5 M^{2}}{9 \delta}\right) \frac{\beta^{2}\left[\beta^{2}+(1+Z)^{2}\right]}{\left(\beta^{2}+1+Z\right)^{2}} . \tag{6.4}
\end{equation*}
$$
...
$$
\begin{equation*}
\alpha=\frac{k T_{e}}{2 E_{I}} \frac{2-f_{I}}{1-f_{I}} \approx \frac{k T_{e}}{E_{I}} \ll 1 . \tag{6.5}
\end{equation*}
$$

Second, the energy balance relation between $\Delta T$ and $\beta, Z$, given in (6.4), shows that for modest to high Hall parameters, $\beta \sim 1$ to $\beta \gg 1$, and typical Mach numbers $M \sim 1-2$, the temperature difference is small for any $Z \lesssim \beta$,

$$
\begin{equation*}
\Delta T=\left(\frac{5 M^{2}}{9 \delta}\right) \frac{\beta^{2}\left[\beta^{2}+(1+Z)^{2}\right]}{\left(\beta^{2}+1+Z\right)^{2}} \sim \frac{1}{\delta} \ll 1 . \tag{6.6}
\end{equation*}
$$
...
Next, to apply the stability criterion, one needs to estimate $\alpha / \Delta T$, the ratio of two small parameters. Since $\alpha \sim 1 / 20$ and $\delta \gtrsim 200$, observe that

$$
\begin{equation*}
\frac{\alpha}{\Delta T} \sim \alpha \delta \gg 1 . \tag{6.7}
\end{equation*}
$$
...
\begin{align*}
\beta^{2} & \leq 4 \alpha\left(2+\frac{1}{\Delta T}\right)\left[1+\alpha\left(1+\frac{1}{\Delta T}\right)\right] \\
& \approx\left(\frac{2 \alpha}{\Delta T}\right)^{2} \gg 1 \rightarrow \beta \leq \frac{2 \alpha}{\Delta T} . \tag{6.8}
\end{align*}
$$
...
$$
\begin{equation*}
\frac{S_{\Omega}}{S_{L}}=\frac{\left[\beta^{2}+(1+Z)^{2}\right]}{\beta^{2} Z} . \tag{6.9}
\end{equation*}
$$
...
$$
\begin{equation*}
Z=\left(1+\beta^{2}\right)^{1 / 2} \sim \beta \tag{6.10}
\end{equation*}
$$
...
$$
\begin{equation*}
\frac{S_{\Omega}}{S_{L}}=2 \frac{1+\left(1+\beta^{2}\right)^{1 / 2}}{\beta^{2}} \sim \frac{1}{\beta} \tag{6.11}
\end{equation*}
$$
...
$$
\begin{equation*}
\sigma=\frac{e^{2} n_{e}}{m_{e} v_{M}} \sim \frac{n_{e}}{T_{e}^{1 / 2}} \tag{6.12}
\end{equation*}
$$

This form is misleading in that raising $n_{e}$ and lowering $T_{e} \approx T_{p}$ to increase $\sigma$ also increases the contributions of $v_{e n}$ and $v_{e i}$ to $v_{M}$. When these contributions are included, it has been shown (Rosa 1987) that there is actually an optimum density that maximises $\sigma$. At the optimum, $n_{e}\left(T_{e}\right)$ and $\sigma\left(T_{e}\right)$ are given by

$$
\begin{align*}
n_{e}^{2}\left(T_{e}\right) & =\left(\frac{p_{p} \bar{\sigma}_{e p}}{\bar{\sigma}_{e n}}\right)\left[\frac{N\left(T_{e}\right)}{k T_{e}}\right] \\
\sigma\left(T_{e}\right) & =\frac{e^{2} n_{e}}{m_{e}} \frac{1}{\left[2\left(v_{e p} v_{e n}\right)^{1 / 2}+v_{e i}\right]} \tag{6.13}
\end{align*}
$$

The conductivity $\sigma_{e}$ is a rapidly increasing function of $T_{e}$ and independent of $B_{0}$.
...
$$
\begin{equation*}
\alpha=\frac{k T_{e}}{2 E_{I}} \frac{2-f_{I}}{1-f_{I}} \approx \frac{k T_{e}}{E_{I}} \ll 1 \tag{6.14}
\end{equation*}
$$
...
\begin{align*}
\alpha & \ll 1, \\
\Delta T \sim \alpha^{1 / 2} & \ll 1, \\
\alpha / \Delta T \sim \alpha^{1 / 2} & \ll 1 . \tag{6.15}
\end{align*}
$$
...
$$
\begin{array}{rlr}
\Delta T \approx \frac{5 M^{2}}{9} \beta^{2} \sim \alpha^{1 / 2} \ll 1 & \text { Energy balance }, \\
\beta^{2} \leq \frac{4 \alpha}{\Delta T} \sim \alpha^{1 / 2} \ll 1 & \text { Stability condition } . \tag{6.16}
\end{array}
$$
...
$$
\begin{array}{rlr}
\frac{S_{\Omega}}{S_{L}} & \approx \frac{(1+Z)^{2}}{\beta^{2} Z} \approx \frac{4}{\beta^{2}} \sim \frac{1}{\alpha^{1 / 2}} \gg 1 & \text { Minimized figure of merit }(Z=1) \\
\sigma\left(T_{e}\right) & =\frac{e^{2} n_{e}}{m_{e}} \frac{1}{\left[2\left(v_{e p} v_{e n}\right)^{1 / 2}+v_{e i}\right]} & T_{e} \approx T_{p} \tag{6.17}
\end{array}
$$
...
$$
\begin{equation*}
\alpha=\frac{k T_{e}}{2 E_{I}} \frac{2-f_{I}}{1-f_{I}} \approx \frac{k T_{e}}{2 E_{I}} \frac{1}{1-f_{I}} \gg 1 . \tag{6.18}
\end{equation*}
$$
...
$$
\begin{equation*}
\beta^{2} \gg Z \gg \beta \gg 1 \tag{6.19}
\end{equation*}
$$
...
$$
\begin{equation*}
\Delta T=\left(\frac{5 M^{2}}{9}\right) \frac{\beta^{2}\left[\beta^{2}+(1+Z)^{2}\right]}{\left(\beta^{2}+1+Z\right)^{2}} \approx\left(\frac{5 M^{2}}{9}\right) \frac{Z^{2}}{\beta^{2}} \gg 1 . \tag{6.20}
\end{equation*}
$$
...
$$
\begin{equation*}
\frac{\alpha}{\Delta T} \approx\left(\frac{9 k T_{e}}{10 E_{I} M^{2}}\right) \frac{\beta^{2}}{Z^{2}} \frac{1}{1-f_{I}} \gg 1, \tag{6.21}
\end{equation*}
$$
...
$$
\begin{equation*}
\beta \leq 2^{1 / 2} \frac{k T_{e}}{E_{I}} \frac{1}{1-f_{I}} \tag{6.23}
\end{equation*}
$$
...
Before proceeding, it is worthwhile noting that so far, it has not been necessary to specify a precise scaling of $Z$ with $\beta$, only the range given by (6.19). As such, a simple way to clarify the abovementioned tangle of orderings is to actually make two specific assumptions with respect to $Z$ and $k T_{e} / E_{I}: Z \sim \beta^{\kappa}$ with $1<\kappa<3 / 2$ and $k T_{e} / E_{I} \sim 1 / \beta$, both consistent with the abovementioned assumptions. Then, all relevant quantities can be scaled directly with $\beta$, and simultaneously satisfy all the orderings previously discussed,

$$
\begin{align*}
\frac{k T_{e}}{E_{I}} & \sim \frac{1}{\beta} \ll 1, \\
Z & \sim \beta^{\kappa} \gg 1 \quad \text { and } \quad 1<\kappa<3 / 2, \\
\Delta T & \sim \beta^{2(\kappa-1)} \gg 1, \\
\alpha & \sim \beta \gg 1, \\
\frac{\alpha}{\Delta T} & \sim \beta^{3-2 \kappa} \gg 1, \\
1-f_{I} & \sim \frac{1}{\beta^{2}} \ll 1 . \tag{6.24}
\end{align*}
$$
...
$$
\begin{equation*}
\frac{S_{\Omega}}{S_{L}}=\frac{\left[\beta^{2}+(1+Z)^{2}\right]}{\beta^{2} Z} \approx \frac{Z}{\beta^{2}} \sim \frac{1}{\beta^{2-\kappa}} \ll 1 . \tag{6.25}
\end{equation*}
$$

If one wants to minimise $S_{\Omega} / S_{L}$, then high $\beta$ (i.e. high $B_{0}$ ) is a good strategy.
The second figure of merit requires a little more work, which makes use of the relations $n_{e} \approx n_{s}, 1-f_{I}=n_{s} / N \approx n_{e} / N$ and $\beta=\left(e B_{0} / m_{e}\right) / \nu_{M}=\Omega_{e} / \nu_{M}$. The result is

$$
\begin{equation*}
\sigma=\frac{e^{2} n_{e}}{m_{e} \nu_{M}}=\left(\frac{e^{2} n_{e}}{m_{e}}\right)\left(\frac{N k T_{e}}{2 E_{I} \alpha}\right) \leq 2^{1 / 2}\left(\frac{k T_{e}}{E_{I}}\right)\left(\frac{e^{2} N}{m_{e} \Omega_{e}}\right) . \tag{6.26}
\end{equation*}
$$

We see that if the goal is to achieve large $\sigma$, then small $\Omega_{e}$ (i.e. small $B_{0}$ ) is the path to take.
...
$$
\begin{equation*}
n_{e}\left(T_{e}\right)=\left(\frac{k T_{e}}{2 E_{I}}\right)\left(\frac{N}{\alpha}\right) \leq 2^{1 / 2}\left(\frac{k T_{e}}{2 E_{I}}\right)\left(\frac{N}{\beta}\right), \tag{6.27}
\end{equation*}
$$
...
The analysis is slightly complicated by the fact that when switching to real units, there is a critical missing piece of information. Ideally, when actually designing
...
Unfortunately, this type of non-local goal is not possible to implement using only a local theory of stability. Instead, a local replacement for the global goal is needed. A good strategy is to assume that the marginal stability criterion is satisfied locally along the entire generator length, and then choose a meaningful physical quantity at the inlet as an alternate local goal. One possible, although not unique, choice for this quantity is the total converted electric power density, $S_{C}$. This quantity strongly impacts economics. Typically, $S_{C}$ in other types of power generators may be of the order of $100 \mathrm{MW} \mathrm{m}^{-3}$. The following results assume that the generator under consideration has a specified value of $S_{C}$ at the inlet, and for comparison, three different values, $S_{C}=100,200,300 \mathrm{MW} \mathrm{m}^{-3}$, are considered.
...
(a) argon coolant input pressure to the Laval nozzle: $p_{\text {in }}=5 \times 10^{6} \mathrm{~Pa} \approx 50 \mathrm{~atm}$;
(b) argon coolant input temperature to the Laval nozzle: $T_{i n}=1000 \mathrm{~K}$;
(c) argon coolant input velocity to the Laval nozzle: highly subsonic $v_{i n}^{2} \ll 2 k T_{i n} / m_{p}$;
...
$$
p_{p}=\left(1+\frac{\gamma-1}{2} M^{2}\right)^{-\frac{\gamma}{\gamma-1}} p_{i n}=\left(\frac{3}{M^{2}+3}\right)^{5 / 2} p_{i n}=0.801 \times 10^{6} \mathrm{~Pa} \approx 8.01 \mathrm{~atm} .
$$

(b) Argon MHD inlet temperature:

$$
T_{p}=\left(1+\frac{\gamma-1}{2} M^{2}\right)^{-1} T_{i n}=\left(\frac{3}{M^{2}+3}\right) T_{i n}=481 \mathrm{~K}
$$

(c) Argon MHD inlet number density:

$$
n_{p}=\frac{p_{p}}{k T_{p}}=\left(1+\frac{\gamma-1}{2} M^{2}\right)^{-\frac{1}{\gamma-1}} \frac{p_{i n}}{k T_{i n}}=\left(\frac{3}{M^{2}+3}\right)^{3 / 2} \frac{p_{i n}}{k T_{i n}}=1.21 \times 10^{26} \mathrm{~m}^{-3} .
$$

(d) Argon MHD inlet velocity:

$$
v_{p}=\left(\frac{\gamma k T_{p} M^{2}}{m_{p}}\right)^{1 / 2}=\left(\frac{5 k T_{p} M^{2}}{3 m_{p}}\right)^{1 / 2}=735 \mathrm{~m} \mathrm{sec}^{-1}
$$
...
$$
\begin{array}{ll}
\Delta T=\frac{T_{e}}{T_{p}}-1 & \text { Definition } \\
\nu_{M}=n_{p} \bar{\sigma}_{e p}\left(\frac{2 k T_{e}}{m_{e}}\right)^{1 / 2} & \text { Definition, } \\
\beta=\frac{e B_{0}}{m_{e} \nu_{M}} & \text { Definition, } \\
Z=\xi+\left[\frac{\xi(\xi+1)}{\mu}\right]^{1 / 2} & \text { Energy balance, } \\
\mu=\frac{9}{5 M^{2} \beta^{2}} \Delta T & \text { Energy balance, } \\
\xi=\frac{\beta^{2} \mu}{1-\mu}-1 & \text { Energy balance, }
\end{array}
$$

![](https://cdn.mathpix.com/cropped/2025_09_02_9f291d0bb2403aa20f63g-27.jpg?height=795&width=1001&top_left_y=201&top_left_x=325)
Figure 2. Figure of merit $S_{\Omega} / S_{L}$ versus magnetic field for three values of $S_{C}\left(\mathrm{MW} \mathrm{m}^{-3}\right)$.

$$
\begin{array}{ll}
n_{e}=\frac{\left(\beta^{2}+1+Z\right)}{\beta^{2}(1+Z)} \frac{S_{C}}{m_{e} \nu_{M} v_{p}^{2}} & \text { Power density, } \\
1-f_{I}=\frac{n_{e} / N\left(T_{e}\right)}{1+n_{e} / N\left(T_{e}\right)} & \text { Saha, } \\
\alpha=\frac{1}{2}\left(\frac{2 k T_{e}}{3 k T_{e}+2 E_{I}}\right)\left(\frac{2-f_{I}}{1-f_{I}}\right) & \text { Definition. } \tag{7.1}
\end{array}
$$
...
$$
\begin{equation*}
\beta^{2}=4 \alpha\left(2+\frac{1}{\Delta T}\right)\left[1+\alpha\left(1+\frac{1}{\Delta T}\right)\right] . \tag{7.2}
\end{equation*}
$$
...
Several conclusions can be drawn. First, $S_{\Omega} / S_{L}$ decreases rapidly with magnetic field, transforming from undesirable values greater than unity to attractive values

![](https://cdn.mathpix.com/cropped/2025_09_02_9f291d0bb2403aa20f63g-28.jpg?height=809&width=996&top_left_y=201&top_left_x=380)
Figure 3. Figure of merit $\sigma$ versus magnetic field for three values of $S_{C}\left(\mathrm{MW} \mathrm{m}^{-3}\right)$.

much less than unity. In other words, high field is a potential winner for improving the efficiency of closed cycle MHD generators operating at the marginal stability boundary of the ionisation instability. Second, the figure of merit is almost independent of the required load power density over a reasonably wide range $100<S_{C}\left(\mathrm{MW} \mathrm{m}^{-3}\right)<300$. This is not surprising since the scaling factor $S_{C}$ cancels when calculating the ratio $S_{\Omega} / S_{L}$. Lastly, as shown by the dashed line, to keep $S_{\Omega} / S_{L}$ below $20 \%$ requires a magnetic field of 14 T or greater. The $20 \%$ value is a desirable practical goal implying that $80 \%$ of the converted electrical power is supplied to the load and only $20 \%$ to heat the electrons.

The conclusions with respect to the second figure of merit $\sigma$, as shown in figure 3, are quite the opposite. Here, lower magnetic field leads to higher conductivity. Also, at any given magnetic field, the conductivity is approximately linearly proportional to the value of $S_{C}$. Higher conductivity requires a higher electron density which, in turn, requires a higher converted power density. Also, in almost all cases, the conductivity is relatively high, greater than $10 \mathrm{mho} \mathrm{m}^{-1}$, which is the requirement for a high-quality plasma even though the primary gas temperature is only 481 K .
...
The electron density decreases rapidly as the field increases. This is a consequence of needing ever lower densities to more closely approach full ionisation, as required by the marginal stability criterion. The density is also a rapidly increasing function

![](https://cdn.mathpix.com/cropped/2025_09_02_9f291d0bb2403aa20f63g-29.jpg?height=1029&width=1289&top_left_y=192&top_left_x=182)
Figure 4. Curves of (a) $n_{e}$, (b) $T_{e}$, (c) $\beta$ and (d) $1-f_{I}$, versus $B_{0}$ for three values of $S_{C}$.

of the power density to the load. This is not surprising - more load power requires more current, which in turn requires more electrons.
...
As a specific reference case, assume values for the critical generator parameters given by $M=1.8, S_{C}=100 \mathrm{MW} \mathrm{m}^{-3}$ and $B_{0}=8 \mathrm{~T}$. It then follows that

$$
\begin{align*}
S_{\Omega} / S_{L} & =0.3720, \\
\sigma & =12.35 \mathrm{mho} \mathrm{~m}^{-1}, \\
T_{e} & =4223 \mathrm{~K}, \\
T_{e} / T_{p} & =8.783, \\
n_{e} & =7.478 \times 10^{19} \mathrm{~m}^{-3}, \\
n_{e} / n_{p} & =6.180 \times 10^{-7}, \\
\beta & =8.249 \\
1-f_{I} & =0.01679 . \tag{7.3}
\end{align*}
$$

As compared with earlier, lower field generators, note the higher temperature ratio and higher Hall parameter.

The last point of interest involves a comparison of results for two other forms of the marginal stability boundary. Keep in mind that the abovementioned results correspond to an exact numerical solution of the stability criterion, but using the approximate form of collision frequency $\nu_{M} \approx \nu_{e p}$. The first comparison involves the $\nu_{M} \approx \nu_{e p}$ approximation. As previously stated, but not proved, it has been assumed that the dominant collision mechanism is between electrons and the primary gas. One can now offer proof by recalculating the results presented in figures 2 and 3, i.e. the curves of $S_{\Omega} / S_{L}$ vs $B_{0}$ and $\sigma$ vs $B_{0}$ for fixed $S_{C}=100 \mathrm{MW} \mathrm{m}^{-3}$. The new curves are obtained by setting (see for instance Wesson 2011 for $v_{e i}$ )

$$
\begin{align*}
v_{M} & =v_{e p}+v_{e n}+v_{e i}, \\
v_{e p} & =n_{p} \bar{\sigma}_{e p} v_{T e}, \\
v_{e n} & =\left(n_{s}-n_{e}\right) \bar{\sigma}_{e n} v_{T e}, \\
v_{e i} & =\frac{2^{1 / 2}}{12 \pi^{3 / 2}} \frac{n_{e} e^{4} \ln \Lambda}{\varepsilon_{0}^{2} m_{e}^{1 / 2}\left(k T_{e}\right)^{3 / 2}} \quad \Lambda=4 \pi \frac{\varepsilon_{0}^{3 / 2}\left(k T_{e}\right)^{3 / 2}}{e^{3} n_{e}^{1 / 2}} \tag{7.4}
\end{align*}
$$

and solving the equations numerically. Results are presented shortly.
The second comparison involves the analytic approximation for the stability boundary. Here, one again assumes $\nu_{M} \approx \nu_{e p}$ and uses the approximate form of the stability boundary given by (6.22), which leads to

$$
\begin{align*}
\frac{S_{\Omega}}{S_{L}} & \approx \frac{Z}{\beta^{2}} \\
\sigma & \approx 2^{1 / 2}\left(\frac{k T_{e}}{E_{I}}\right)\left(\frac{e^{2} N}{m_{e} \Omega_{e}}\right) . \tag{7.5}
\end{align*}
$$
...
$$
\begin{array}{ll}
\Delta T \approx \frac{T_{e}}{T_{p}} & \text { Definition, } \\
v_{M} \approx n_{p} \bar{\sigma}_{e p}\left(\frac{2 k T_{e}}{m_{e}}\right)^{1 / 2} & \text { Definition, } \\
\beta=\frac{e B_{0}}{m_{e} v_{M}} & \text { Definition, } \\
Z \approx\left(\frac{9 \Delta T}{5 M^{2}}\right)^{1 / 2} \beta & \text { Energy balance, } \\
n_{e} \approx \frac{S_{C}}{m_{e} v_{M} v_{p}^{2} Z} & \text { Power density, } \\
1-f_{I} \approx n_{e} / N\left(T_{e}\right) & \text { Saha, } \\
\alpha \approx \frac{k T_{e}}{2 E_{I}}\left(\frac{1}{1-f_{I}}\right) & \text { Definition, } \\
\beta^{2} \approx 8 \alpha^{2} & \text { Marginal stability. } \tag{7.6}
\end{array}
$$
...
w^{-7 / 2} \exp (-w) & =C \\
w & =\frac{E_{I}}{k T_{e}} \\
C & =\left[\frac{1}{\left(160 \pi^{3}\right)^{1 / 2}} \frac{m_{p} h^{3} k^{1 / 2}}{m_{e}^{2} E_{I}^{5 / 2} \bar{\sigma}_{e p}}\right]\left(\frac{T_{p}^{1 / 2} S_{C}}{M p_{p}}\right)=7.715 \times 10^{-13}\left(\frac{T_{p}^{1 / 2} S_{C}}{M p_{p}}\right) \tag{7.7}
\end{align*}
$$
...
$$
\begin{align*}
\frac{S_{\Omega}}{S_{L}} & =\left[\left(\frac{18}{5}\right)^{1 / 2} \frac{m_{e}^{1 / 2} \bar{\sigma}_{e p} E_{I}}{e k^{3 / 2}}\right] \frac{p_{p}}{w T_{p}^{3 / 2} M B_{0}}=0.6050 \frac{p_{p}}{w T_{p}^{3 / 2} M B_{0}} \\
\sigma & =\left[4 \pi^{3 / 2} \frac{e\left(m_{e} E_{I}\right)^{3 / 2}}{h^{3}}\right] \frac{w^{-5 / 2} e^{-w}}{B_{0}}=6.184 \frac{w^{-5 / 2} e^{-w}}{B_{0}} \tag{7.8}
\end{align*}
$$
...
One main contribution is a first-principles derivation that shows the direct connection between stabilisation of the mode and a high seed-ionisation fraction. Here,

![](https://cdn.mathpix.com/cropped/2025_09_02_9f291d0bb2403aa20f63g-32.jpg?height=794&width=1008&top_left_y=199&top_left_x=372)
Figure 5. Comparisons of $S_{\Omega} / S_{L}$ for three different stability models.

stabilisation refers to the largest allowable value of the Hall parameter $\beta$. A specific expression has been derived that predicts the maximum allowable seed density to achieve the desired high value of stable $\beta$. High values of $\beta$ imply a large ratio of $T_{e} / T_{p}$, of the order of 10, which is stable and very important in producing a highly conducting plasma when the primary gas is relatively cold. Such large ratios have not been observed in earlier experiments. It would indeed be interesting to carry out an experimental programme demonstrating these large stable values of $T_{e} / T_{p}$.
...
What is the bottom line? Overall, the access to higher temperature ratios by means of further increasing the ionisation towards $f_{I} \rightarrow 1$ is a good result, suggesting that high-quality plasmas are possible in closed cycle MHD generators for both high and low magnetic fields. The desirability of high field is undecided with the two figures of merit giving opposing conclusions. So, what is the answer to the question in the

![](https://cdn.mathpix.com/cropped/2025_09_02_9f291d0bb2403aa20f63g-33.jpg?height=816&width=991&top_left_y=199&top_left_x=332)
Figure 6. Comparisons of $\sigma$ for three different stability models.

title of the paper 'A New Opportunity for MHD Energy Conversion?'. The answer is 'maybe'. New work has been completed that quantitatively addresses the global performance of MHD generators operating in the nearly fully ionised mode. Results will be presented in a future paper.
...
Hatori, S. \& Shioda, S. 1974 Stabilization of the ionization instability in an MHD generator. J. Phys. Soc. Jpn. 36, 920-920.
KARLOVITZ, B. 1940 Process for the conversion of energy. U.S. Patent No. 2,210,918.

Kerrebrock, J.L. 1964 Nonequilibrium ionization due to electron heating: I Theory. AAIA J. 2, 10721080.

Kerrebrock, J.L. \& Hoffman, M.A. 1964 Nonequilibrium ionization due to electron heating: II Experiments. AAIA Journal 2, 1080-1087.
Kien, L.C. 2016 Analyses on the ionization instability of non-equilibrium seeded plasma in an MHD generator. Plasma Sci. Technol. 18, 674-679.
Messerle, H.K. 1994 Magnetohydrodynamic Electric Power Generation. John Wiley \& Sons.
Mitchner, M. \& Kruger, C.H. 1973 Partially Ionize Gases. John Wiley \& Sons.
Murakami, T., Okuno, Y. \& Yamasaki, H. 2005 Suppression of ionization instability in a magnetohydrodynamic plasma by coupling with a radio-frequency electromagnetic field. Appl. Phys. Lett. 86, 191502.
NAKAMURA, T. \& RIEDMULLER, W. 1974 Stability of nonequilibrium MHD plasma in the regime of fully ionized seed. AIAA J. 12, 661-668.
NEA 2022 High-Temperature Gas-Cooled Reactors and Industrial Heat Applications. OECD Publishing.
Petit, J.P., Caressa, J.P. \& Valensi, J. 1968 Theoretical and experimental study, using a shock tube, of the phenomena accompanying equilibrium breakdown in a closed cycle MHD generator. In Electricity from MHD: Proceedings of a Symposium on magnetohydrodynamic electric power generation (in French), vol. 2, p. 745.
Petit, J.P. \& Geffray, J. 2008 Non-equilibrium plasma instabilities. In Proceedings of the, 2nd EuroAsian Pulsed Power Conference, p. 1170.
Petit, J.P. \& VALENSI, J. 1969 Growth rate of electrothermal instability and critical Hall parameter in closed-cycle MHD generators when the electron mobility is variable. Compt. Rend. Acad. Sci. 269, 365.

Rosa, R.J. 1987 Magnetohydrodynamic Energy Conversion. Hemisphere Publishing Corporation.
Sheindlin, A.E., Batenin, V.A. \& Asinovsky, E.I. 1964 Investigation of non-equilibrium ionization in a mixture of argon and potassium. In International Symposium on Magnetohydrodynamic Electric Power Generation, Paris, France.
Sorbom, B.M., et al. 2015 ARC: a compact, high-field, fusion nuclear science facility and demonstration power plant with demountable magnets. Fusion Eng. Des. 100, 378-405.
Sporn, P. \& Kantrowitz, A. 1959 Large-scale generation of electric power by application of the magnetohydrodynamic concept. Power 103, 62.
Velikhov, E.P. 1962 Hall instability of current carrying slightly ionized plasmas. In 1st International Conference on MHD Electrical Power Generation, Newcastle upon Tyne, England, p. 135.
Velikhov, E.P. \& Dykhne, A.M. 1963 Plasma turbulence due to the ionization instability in a strong magnetic field. In Proceedings of the, 6th International Conference on Phenomena in Ionized Gases, vol. 4511, p. 511.
Velikhov, E.P., Dykhne, A.M. \& Shipuk, I.YA 1965 Ionization instability of a plasma with hot electrons. In: Proceedings of the, 7th International Conference on Phenomena in Ionized Gases.
Vieira, R.F., et al. 2024 Design, fabrication, and assembly of the SPARC toroidal field model coil. IEEE Trans. Appl. Supercon. 34, 0600615-15.
Wesson, J. 2011 Tokamaks Fourth Edition. Oxford University Press.
